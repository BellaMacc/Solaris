...
AFRAME.registerComponent('solaris-pickup-object', {
  ...
  receiveSlotRequest: function(slotContext){
    //this function exists serperately from parent to slot because 
    //it has the added functionality of setting the object slot's held item element. 
    
    //constants
    const CONTEXT_AF = this;
    
     //debug notif
    console.log("Slot-placing request received from: " +  slotContext.el.getAttribute('id')+ ", my id is " + CONTEXT_AF.el.getAttribute('id'));
    
    if (CONTEXT_AF.pickedUp === true) {
      //debug notif
      console.log(("I will be parented to " + slotContext));

      //let the object slot know you exist
  ...
  getContext: function(){
    //allows other components to work with this data
    return this;
  },
});